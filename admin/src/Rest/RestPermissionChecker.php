...
class RestPermissionChecker
{
  ...
  private static function validate_basic_auth($request)
  {
    ...
    if (!$auth_header || strpos($auth_header, 'Basic ') !== 0) {
      return new \WP_Error(
        'rest_forbidden',
        __('Authentication required. Please provide Basic Auth credentials.', 'flexify-dashboard'),
        ['status' => 401]
      );
    }
      ...
      if (is_array($required_capability)) {
        ...
        foreach ($required_capability as $cap) {
          if (!current_user_can($cap)) {
            return new \WP_Error(
              'rest_forbidden',
              sprintf(__('You do not have permission to perform this action. Required capability: %s', 'flexify-dashboard'), $cap),
              ['status' => 403]
            );
          }
        }
        ...
        if (!current_user_can($required_capability)) {
          return new \WP_Error(
            'rest_forbidden',
            sprintf(__('You do not have permission to perform this action. Required capability: %s', 'flexify-dashboard'), $required_capability),
            ['status' => 403]
          );
        }
      }
