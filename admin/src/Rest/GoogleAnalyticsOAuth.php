...
class GoogleAnalyticsOAuth
{
    ...
    public function register_routes(): void
    {
        ...
        register_rest_route('flexify-dashboard/v1', '/google-analytics/credentials', [
            'methods' => 'POST',
        ...
        register_rest_route('flexify-dashboard/v1', '/google-analytics/disconnect', [
            'methods' => 'POST',
        ...
        register_rest_route('flexify-dashboard/v1', '/google-analytics/properties', [
            'methods' => 'GET',
        ...
        register_rest_route('flexify-dashboard/v1', '/google-analytics/status', [
            'methods' => 'GET',
        ...
        register_rest_route('flexify-dashboard/v1', '/google-analytics/property', [
            'methods' => 'POST',
        ...
        register_rest_route('flexify-dashboard/v1', '/google-analytics/validate', [
            'methods' => 'POST',
    ...
    public function validate_credentials($request)
    {
        ...
        if (!$credentials) {
            return new \WP_Error('invalid_json', __('Invalid JSON format', 'flexify-dashboard'), ['status' => 400]);
        }
        ...
        foreach ($required_fields as $field) {
            if (empty($credentials[$field])) {
                return new \WP_Error('missing_field', sprintf(__('Missing required field: %s', 'flexify-dashboard'), $field), ['status' => 400]);
            }
        }

        if ($credentials['type'] !== 'service_account') {
            return new \WP_Error('invalid_type', __('JSON must be a service account key (type should be "service_account")', 'flexify-dashboard'), ['status' => 400]);
        }

        // Try to get an access token to validate the credentials
        $accessToken = $this->getAccessTokenFromCredentials($credentials);
        if (!$accessToken) {
            return new \WP_Error('auth_failed', __('Failed to authenticate with Google. Please check your service account key.', 'flexify-dashboard'), ['status' => 400]);
        }
    ...
    public function save_credentials($request)
    {
        ...
        if ($credentials['type'] !== 'service_account') {
            return new \WP_Error('invalid_type', __('JSON must be a service account key', 'flexify-dashboard'), ['status' => 400]);
        }
        ...
        delete_transient('flexify_dashboard_ga_access_token');
    ...
    public function get_properties($request)
    {
        ...
        if (!$accessToken) {
            return new \WP_Error('not_connected', __('Not connected to Google Analytics', 'flexify-dashboard'), ['status' => 401]);
        }
        ...
        if ($status_code !== 200) {
            $error_message = $body['error']['message'] ?? __('Failed to fetch properties. Make sure the service account has been added to your GA4 property.', 'flexify-dashboard');
    ...
    public function save_property($request)
    {
        ...
        if (empty($property_id)) {
            return new \WP_Error('missing_property', __('Property ID is required', 'flexify-dashboard'), ['status' => 400]);
        }
    ...
    {
        // Check cached token
        $cachedToken = get_transient('flexify_dashboard_ga_access_token');
        if ($cachedToken) {
            return $cachedToken;
        }
    ...
    private function getAccessTokenFromCredentials(array $credentials): ?string
    {
        ...
        if (isset($body['access_token'])) {
            ...
            set_transient('flexify_dashboard_ga_access_token', $body['access_token'], 3000);
    ...
    /**
     * Update flexifyDashboard settings
     * 
