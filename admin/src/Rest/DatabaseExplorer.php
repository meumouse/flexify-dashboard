...
class DatabaseExplorer
{
  ...
  public static function register_custom_endpoints()
  {
    ...
    register_rest_route("flexify-dashboard/v1", "/database/tables", [
      "methods" => "GET",
      "callback" => ["MeuMouse\Flexify_Dashboard\Rest\DatabaseExplorer", "get_tables"],
      "permission_callback" => ["MeuMouse\Flexify_Dashboard\Rest\DatabaseExplorer", "check_permissions"],
    ]);

    // Get table structure (columns, indexes, etc.)
    register_rest_route("flexify-dashboard/v1", "/database/tables/(?P<table>[a-zA-Z0-9_]+)/structure", [
      "methods" => "GET",
      "callback" => ["MeuMouse\Flexify_Dashboard\Rest\DatabaseExplorer", "get_table_structure"],
      "permission_callback" => ["MeuMouse\Flexify_Dashboard\Rest\DatabaseExplorer", "check_permissions"],
      "args" => [
        "table" => [
          "required" => true,
    ...
    register_rest_route("flexify-dashboard/v1", "/database/tables/(?P<table>[a-zA-Z0-9_]+)/data", [
      "methods" => "GET",
      "callback" => ["MeuMouse\Flexify_Dashboard\Rest\DatabaseExplorer", "get_table_data"],
      "permission_callback" => ["MeuMouse\Flexify_Dashboard\Rest\DatabaseExplorer", "check_permissions"],
      "args" => [
        "table" => [
          "required" => true,
    ...
    register_rest_route("flexify-dashboard/v1", "/database/query", [
      "methods" => "POST",
      "callback" => ["MeuMouse\Flexify_Dashboard\Rest\DatabaseExplorer", "execute_query"],
      "permission_callback" => ["MeuMouse\Flexify_Dashboard\Rest\DatabaseExplorer", "check_permissions"],
      "args" => [
        "query" => [
          "required" => true,
          ...
          "sanitize_callback" => function ($param) {
            ...
            if (stripos($trimmed, "SELECT") !== 0) {
              return new \WP_Error("invalid_query", __("Only SELECT queries are allowed for safety.", "flexify-dashboard"), ["status" => 400]);
            }
            return $param;
          },
        ],
    ...
    // Get table row count
    register_rest_route("flexify-dashboard/v1", "/database/tables/(?P<table>[a-zA-Z0-9_]+)/count", [
      "methods" => "GET",
      "callback" => ["MeuMouse\Flexify_Dashboard\Rest\DatabaseExplorer", "get_table_count"],
      "permission_callback" => ["MeuMouse\Flexify_Dashboard\Rest\DatabaseExplorer", "check_permissions"],
      "args" => [
        "table" => [
          "required" => true,
    ...
    register_rest_route("flexify-dashboard/v1", "/database/verify-password", [
      "methods" => "POST",
      "callback" => ["MeuMouse\Flexify_Dashboard\Rest\DatabaseExplorer", "verify_password"],
      "permission_callback" => ["MeuMouse\Flexify_Dashboard\Rest\DatabaseExplorer", "check_permissions"],
      "args" => [
        "password" => [
          "required" => true,
    ...
    register_rest_route("flexify-dashboard/v1", "/database/tables/(?P<table>[a-zA-Z0-9_]+)", [
      "methods" => "DELETE",
      "callback" => ["MeuMouse\Flexify_Dashboard\Rest\DatabaseExplorer", "delete_table"],
      "permission_callback" => ["MeuMouse\Flexify_Dashboard\Rest\DatabaseExplorer", "check_permissions"],
      "args" => [
        "table" => [
          "required" => true,
  ...
  public static function get_table_structure($request)
  {
    ...
    $validated_table_name = self::validate_table_name($table_name);
    if (!$validated_table_name) {
      return new \WP_Error("invalid_table_name", __("Invalid table name.", "flexify-dashboard"), ["status" => 400]);
    }

    // Validate table name exists
    if (!$wpdb->get_var($wpdb->prepare("SHOW TABLES LIKE %s", $table_name))) {
      return new \WP_Error("table_not_found", __("Table not found.", "flexify-dashboard"), ["status" => 404]);
    }
  ...
  public static function get_table_data($request)
  {
    ...
    if (empty($columns)) {
      return new \WP_Error("table_error", __("Unable to retrieve table structure.", "flexify-dashboard"), ["status" => 500]);
    }
  ...
  public static function execute_query($request)
  {
    ...
    foreach ($dangerous_keywords as $keyword) {
      if (stripos($query, $keyword) !== false && stripos($query, "SELECT") === false) {
        return new \WP_Error("invalid_query", __("Query contains prohibited SQL keywords.", "flexify-dashboard"), ["status" => 400]);
      }
    }
    ...
    if (count($parts) > 1) {
      ...
      for ($i = 1; $i < count($parts); $i++) {
        $part = trim($parts[$i]);
        if (!empty($part)) {
          return new \WP_Error("invalid_query", __("Multiple statements are not allowed.", "flexify-dashboard"), ["status" => 400]);
        }
      }
    }
    ...
    if ($wpdb->last_error) {
      ...
      return new \WP_Error("query_error", __("Query execution failed. Please check your SQL syntax.", "flexify-dashboard"), ["status" => 500]);
    }
  ...
  public static function verify_password($request)
  {
    ...
    if (empty($password)) {
      return new \WP_Error("password_required", __("Password is required.", "flexify-dashboard"), ["status" => 400]);
    }
    ...
    if (is_wp_error($user)) {
      return new \WP_Error("invalid_password", __("Invalid password.", "flexify-dashboard"), ["status" => 403]);
    }

    // Verify it's the current user
    if ($user->ID !== $current_user->ID) {
      return new \WP_Error("user_mismatch", __("Password verification failed.", "flexify-dashboard"), ["status" => 403]);
    }
  ...
  public static function delete_table($request)
  {
    ...
    // Verify password
    if (empty($password)) {
      return new \WP_Error("password_required", __("Password is required for this operation.", "flexify-dashboard"), ["status" => 400]);
    }
    ...
    if ($result === false) {
      return new \WP_Error("deletion_failed", __("Failed to delete table.", "flexify-dashboard"), ["status" => 500]);
    }
