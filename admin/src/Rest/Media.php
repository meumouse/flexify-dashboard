...
class Media {
	...
	public static function filter_unused_attachments( array $args, WP_REST_Request $request ): array {
		...
		$featured_ids = get_transient( 'flexify_dashboard_featured_attachment_ids_cache' );
		if ( false === $featured_ids ) {
			...
			set_transient( 'flexify_dashboard_featured_attachment_ids_cache', $featured_ids, MINUTE_IN_SECONDS * 5 );
		...
		$attached_ids = get_transient( 'flexify_dashboard_attached_attachment_ids' );
		if ( false === $attached_ids ) {
			...
			set_transient( 'flexify_dashboard_attached_attachment_ids', $attached_ids, MINUTE_IN_SECONDS * 5 );
	...
	private static function maybe_apply_custom_orderby( array $args, WP_REST_Request $request ): array {
		...
		if ( 'size' === $orderby ) {
			...
			add_filter( 'posts_clauses', function ( $clauses ) use ( $order ) {
				global $wpdb;
				$join  = " LEFT JOIN {$wpdb->postmeta} AS ufsm ON ( {$wpdb->posts}.ID = ufsm.post_id AND ufsm.meta_key = 'flexify_dashboard_filesize' ) ";
				$clauses['join'] .= $join;
			...
			add_filter( 'the_posts', function ( $posts ) {
				foreach ( $posts as $p ) {
					...
					$existing = get_post_meta( $p->ID, 'flexify_dashboard_filesize', true );
					...
					if ( $path && file_exists( $path ) ) {
						...
						if ( $bytes > 0 ) {
							update_post_meta( $p->ID, 'flexify_dashboard_filesize', $bytes );
						}
					}
				}
				return $posts;
	...
	private static function collect_used_attachment_ids_from_content(): array {
		$cache_key = 'flexify_dashboard_used_attachment_ids_content_cache';
