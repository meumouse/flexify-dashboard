...
class AttachmentMetaBoxManager
{
  ...
  public function add_attachment_fields($form_fields, $post)
  {
    ...
    foreach ($field_groups as $group) {
      ...
      // Add as a form field
      $form_fields['flexify_dashboard_' . $group['id']] = [
        'label' => $group['title'],
        'input' => 'html',
        'html' => sprintf(
          '<div class="flexify-dashboard-custom-fields-app flexify-dashboard-attachment-fields" data-field-group="%s" data-saved-values="%s" data-attachment-id="%d" data-context="attachment"></div>',
  ...
  public function add_attachment_meta_box($post)
  {
    ...
    foreach ($field_groups as $group) {
      add_meta_box(
        'flexify_dashboard_attachment_' . $group['id'],
        $group['title'],
  ...
  public function render_attachment_meta_box($post, $args)
  {
    ...
    // Nonce for security
    wp_nonce_field('flexify_dashboard_attachment_fields_' . $group['id'], 'flexify_dashboard_attachment_nonce_' . $group['id']);
  ...
  public function save_attachment_fields($post, $attachment)
  {
    ...
    $custom_fields = isset($attachment['flexify_dashboard_cf']) ? $attachment['flexify_dashboard_cf'] : [];
