...
class CommentMetaBoxManager
{
  ...
  public function add_comment_meta_box($comment)
  {
    ...
    foreach ($field_groups as $group) {
      add_meta_box(
        'flexify_dashboard_comment_' . $group['id'],
        $group['title'],
  ...
  public function render_comment_fields($comment, $args)
  {
    ...
    // Nonce for security
    wp_nonce_field('flexify_dashboard_comment_fields_' . $group['id'], 'flexify_dashboard_comment_nonce_' . $group['id']);
    ...
    printf(
      '<div class="flexify-dashboard-custom-fields-app flexify-dashboard-comment-fields" data-field-group="%s" data-saved-values="%s" data-comment-id="%d" data-context="comment"></div>',
  ...
  public function save_comment_fields($comment_id)
  {
    ...
    foreach ($field_groups as $group) {
      ...
      if (isset($_POST[$nonce_name]) && wp_verify_nonce($_POST[$nonce_name], 'flexify_dashboard_comment_fields_' . $group['id'])) {
