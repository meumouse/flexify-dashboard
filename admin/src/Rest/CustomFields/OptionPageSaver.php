...
use MeuMouse\Flexify_Dashboard\Rest\CustomFields\FieldValueSanitizer;
...
class OptionPageSaver
{
  ...
  public function handle_save()
  {
    ...
    if (!isset($_POST['flexify_dashboard_option_page_nonce']) || !isset($_POST['flexify_dashboard_option_page_slug'])) {
      return;
    }

    $page_slug = sanitize_key($_POST['flexify_dashboard_option_page_slug']);

    // Verify nonce
    if (!wp_verify_nonce($_POST['flexify_dashboard_option_page_nonce'], 'flexify_dashboard_option_page_' . $page_slug)) {
      wp_die(__('Security check failed.', 'flexify-dashboard'));
    }

    // Check permissions
    if (!current_user_can('manage_options')) {
      wp_die(__('You do not have permission to save these options.', 'flexify-dashboard'));
    }
    ...
    $custom_fields = isset($_POST['flexify_dashboard_cf']) ? $_POST['flexify_dashboard_cf'] : [];

    // Build options array
    $option_key = 'flexify_dashboard_options_' . $page_slug;
