...
class OptionPages
{
  ...
  public function get_option_page($request)
  {
    ...
    if (!$this->is_valid_slug($slug)) {
      return new \WP_Error('rest_invalid_slug', __('Invalid option page slug.', 'flexify-dashboard'), ['status' => 400]);
    }

    $page = $this->repository->find_by_slug($slug);
    
    if ($page === null) {
      return new \WP_Error('rest_not_found', __('Option page not found.', 'flexify-dashboard'), ['status' => 404]);
    }
  ...
  public function create_option_page($request)
  {
    ...
    if (empty($body['title'])) {
      return new \WP_Error('rest_missing_title', __('Option page title is required.', 'flexify-dashboard'), ['status' => 400]);
    }
    ...
    if (empty($new_page['slug'])) {
      ...
      if ($this->repository->slug_exists($new_page['slug'], $option_pages)) {
        return new \WP_Error('rest_slug_exists', __('An option page with this slug already exists.', 'flexify-dashboard'), ['status' => 400]);
      }
    }
    ...
    if (!$this->repository->write($option_pages)) {
      return new \WP_Error('rest_save_failed', __('Failed to save option page.', 'flexify-dashboard'), ['status' => 500]);
    }
  ...
  public function delete_option_page($request)
  {
    ...
    // Save to file
    if (!$this->repository->write($option_pages)) {
      return new \WP_Error('rest_save_failed', __('Failed to delete option page.', 'flexify-dashboard'), ['status' => 500]);
    }
  ...
  public function duplicate_option_page($request)
  {
    ...
    // Save to file
    if (!$this->repository->write($option_pages)) {
      return new \WP_Error('rest_save_failed', __('Failed to duplicate option page.', 'flexify-dashboard'), ['status' => 500]);
    }
