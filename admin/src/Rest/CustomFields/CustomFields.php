...
use MeuMouse\Flexify_Dashboard\Rest\CustomFields\FieldGroupRepository;
use MeuMouse\Flexify_Dashboard\Rest\CustomFields\FieldGroupSanitizer;
use MeuMouse\Flexify_Dashboard\Rest\CustomFields\LocationDataProvider;
use MeuMouse\Flexify_Dashboard\Rest\CustomFields\LocationRuleEvaluator;
use MeuMouse\Flexify_Dashboard\Rest\CustomFields\MetaBoxManager;
use MeuMouse\Flexify_Dashboard\Rest\CustomFields\FieldSaver;
...
class CustomFields
{
  ...
  {
    $json_file_path = WP_CONTENT_DIR . '/flexify-dashboard-custom-fields.json';
  ...
  public function get_field_group($request)
  {
    ...
    if (!$this->sanitizer->is_valid_id($id)) {
      return new \WP_Error('rest_invalid_id', __('Invalid field group ID.', 'flexify-dashboard'), ['status' => 400]);
    }

    $group = $this->repository->find_by_id($id);
    
    if ($group === null) {
      return new \WP_Error('rest_not_found', __('Field group not found.', 'flexify-dashboard'), ['status' => 404]);
    }
  ...
  public function create_field_group($request)
  {
    ...
    if (empty($body['title'])) {
      return new \WP_Error('rest_missing_title', __('Field group title is required.', 'flexify-dashboard'), ['status' => 400]);
    }
    ...
    if (!$this->repository->write($field_groups)) {
      return new \WP_Error('rest_save_failed', __('Failed to save field group.', 'flexify-dashboard'), ['status' => 500]);
    }
  ...
  public function delete_field_group($request)
  {
    ...
    // Save to file
    if (!$this->repository->write($field_groups)) {
      return new \WP_Error('rest_save_failed', __('Failed to delete field group.', 'flexify-dashboard'), ['status' => 500]);
    }
  ...
  public function duplicate_field_group($request)
  {
    ...
    // Save to file
    if (!$this->repository->write($field_groups)) {
      return new \WP_Error('rest_save_failed', __('Failed to duplicate field group.', 'flexify-dashboard'), ['status' => 500]);
    }
  ...
  public function import_field_groups($request)
  {
    ...
    if (empty($body['data']) || !is_array($body['data'])) {
      return new \WP_Error('rest_invalid_data', __('Invalid import data. Expected an array of field groups.', 'flexify-dashboard'), ['status' => 400]);
    }
    ...
    foreach ($import_data as $index => $group) {
      ...
      if (empty($group['title'])) {
        $errors[] = sprintf(__('Field group at index %d is missing a title.', 'flexify-dashboard'), $index);
        continue;
      }
    ...
    if (!$this->repository->write($existing_groups)) {
      return new \WP_Error('rest_save_failed', __('Failed to save imported field groups.', 'flexify-dashboard'), ['status' => 500]);
    }
  ...
  public function preview_field_value($request)
  {
    ...
    if (!in_array($function, self::$allowed_preview_functions, true)) {
      return new \WP_Error(
        ...
        __('Invalid or unauthorized function.', 'flexify-dashboard'),
        ['status' => 400]
      );
    }
    ...
    if (empty($field_name)) {
      return new \WP_Error(
        ...
        __('Field name is required.', 'flexify-dashboard'),
        ['status' => 400]
      );
    }
    ...
    if ($context === 'option') {
      if (empty($option_page)) {
        return new \WP_Error(
          ...
          __('Valid option page is required.', 'flexify-dashboard'),
          ['status' => 400]
        );
      }
    } else {
      if ($object_id <= 0) {
        return new \WP_Error(
          ...
          __('Valid object ID is required.', 'flexify-dashboard'),
          ['status' => 400]
        );
      }
    }
  ...
  public function search_posts($request)
  {
    ...
    foreach ($query->posts as $post) {
      $results[] = [
        'id' => $post->ID,
        'title' => $post->post_title ?: __('(no title)', 'flexify-dashboard'),
