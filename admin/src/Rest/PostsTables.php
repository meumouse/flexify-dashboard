...
class PostsTables
{
  ...
  private function process_date_column($post)
  {
    ...
    if (get_post_status($post) === "future") {
      return sprintf('<span title="%s">%s</span>', esc_attr($exact_date), sprintf(__("Scheduled for %s from now", "flexify-dashboard"), $time_diff));
    }

    return sprintf('<span title="%s">%s</span>', esc_attr($exact_date), sprintf(__("%s ago", "flexify-dashboard"), $time_diff));
  }
  ...
  private function process_comments_column($post)
  {
    ...
    return $comment_count > 0 ? "<span class='fd-comment-count'>{$comment_count}</span>" : "";
  }
  ...
  {
    return isset($_GET["page"]) && $_GET["page"] === "flexify-dashboard-posts-data";
  }
  ...
  public function render_page()
  {
    ...
    ?>
        <script type="application/json" id="flexify-dashboard-posts-data">
