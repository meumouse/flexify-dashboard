...
use MeuMouse\Flexify_Dashboard\Options\TextReplacement;
use MeuMouse\Flexify_Dashboard\Options\AdminFavicon;
use MeuMouse\Flexify_Dashboard\Options\LoginOptions;
use MeuMouse\Flexify_Dashboard\Options\MediaOptions;
...
{
  /**
   * Returns the default Flexify Dashboard settings.
   *
   * @return array
   */
  public static function get_default_settings()
  {
    return [
      ...
      "hidden_plugin_update_notifications" => [],
      "show_hidden_plugin_update_notifications_for" => [],
  ...
  {
    add_action("admin_init", ["MeuMouse\Flexify_Dashboard\Options\GlobalOptions", "create_global_option"]);
    add_action("rest_api_init", ["MeuMouse\Flexify_Dashboard\Options\GlobalOptions", "create_global_option"]);
  ...
  public static function create_global_option()
  {
    $args = [
      "type" => "object",
      "sanitize_callback" => ["MeuMouse\Flexify_Dashboard\Options\GlobalOptions", "sanitize_global_settings"],
  ...
  public static function sanitize_global_settings($value)
  {
	...
	if (isset($value["modern_login_logo"])) {
      $sanitized_value["modern_login_logo"] = sanitize_text_field($value["modern_login_logo"]);
    }
    ...
    if (isset($value["enable_flexify_dashboard_analytics"])) {
      $sanitized_value["enable_flexify_dashboard_analytics"] = (bool) $value["enable_flexify_dashboard_analytics"];
    }

    if (isset($value["analytics_provider"])) {
      $allowed_providers = ["flexify-dashboard", "google_analytics"];
    ...
    if (isset($value["show_hidden_plugin_update_notifications_for"]) && is_array($value["show_hidden_plugin_update_notifications_for"])) {
      $formattedAccessList = [];
      foreach ($value["show_hidden_plugin_update_notifications_for"] as $entry) {
        if (is_array($entry)) {
          $sanitized_entry = [
            "id" => isset($entry["id"]) ? (int) sanitize_text_field($entry["id"]) : "",
            "value" => isset($entry["value"]) ? sanitize_text_field($entry["value"]) : "",
            "type" => isset($entry["type"]) ? sanitize_text_field($entry["type"]) : "",
          ];
          $formattedAccessList[] = $sanitized_entry;
        }
      }
      $sanitized_value["show_hidden_plugin_update_notifications_for"] = $formattedAccessList;
    }
