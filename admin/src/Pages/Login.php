...
class Login
{
  ...
  public static function start_login_wrapper()
  {
    ...
    $wrapper = '
		...
		<div id="flexify-dashboard-login-wrap">
			<div id="flexify-dashboard-login-form-wrap">
				<div id="flexify-dashboard-login-form">';
  ...
  public static function end_login_wrapper()
  {
    ...
    $wrapper = '
	  		...
	  		<div id="flexify-dashboard-login-panel"></div>
	  </div>
	  <!-- END OF UIP WRAP -->
	 ';
  ...
  private static function build_custom_properties($scale, $colorName)
  {
    ...
    foreach ($scale as $color) {
      ...
      $propertyName = "--fd-{$colorName}-{$escaped_step}";
  ...
  public static function load_styles()
  {
    ...
    // Get plugin root url and load bundled css with a fallback for dev/source distributions
    ...
    $style_relative = file_exists($plugin_path . "/app/dist/assets/styles/login.css")
      ? "app/dist/assets/styles/login.css"
      : "app/src/apps/styles/login.css";

    $style = $url . $style_relative;
    ...
    $logo = isset(self::$options["modern_login_logo"]) && self::$options["modern_login_logo"] != ""
      ? esc_html(self::$options["modern_login_logo"])
      : (isset(self::$options["logo"]) && self::$options["logo"] != "" ? esc_html(self::$options["logo"]) : false);
