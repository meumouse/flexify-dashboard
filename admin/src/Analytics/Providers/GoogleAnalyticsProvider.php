...
class GoogleAnalyticsProvider implements AnalyticsProviderInterface
{
    ...
    private function getAccessToken(): ?string
    {
        ...
        if (isset($body['access_token'])) {
            // Cache the token
            set_transient('flexify_dashboard_ga_access_token', $body['access_token'], self::TOKEN_CACHE_DURATION);
    ...
    private function makeApiRequest(string $url, array $body): array
    {
        ...
        if (!$access_token) {
            $this->setLastError('no_access_token', __('Could not authenticate with Google Analytics. Please check your service account credentials.', 'flexify-dashboard'));
            return [];
        }
        ...
        if ($status_code !== 200) {
            ...
            if ($status_code === 403 && $error_status === 'PERMISSION_DENIED') {
                $user_message = __('Permission denied: The service account does not have access to this Google Analytics property. Please add the service account email to your GA4 property with Viewer access. Go to Google Analytics → Admin → Property Access Management and add the service account email.', 'flexify-dashboard');
                ...
                $this->setLastError('authentication_failed', __('Authentication failed. The access token is invalid or expired. Please try reconnecting your Google Analytics account.', 'flexify-dashboard'));
                $this->refreshAccessToken();
            } elseif ($status_code === 404) {
                $this->setLastError('property_not_found', __('The specified Google Analytics property was not found. Please check that the Property ID is correct.', 'flexify-dashboard'));
            } else {
    ...
    private function setLastError(string $code, string $message, array $extra = []): void
    {
        ...
        set_transient('flexify_dashboard_ga_last_error', $error, HOUR_IN_SECONDS);
    }
    ...
    {
        delete_transient('flexify_dashboard_ga_last_error');
    }
    ...
    {
        return get_transient('flexify_dashboard_ga_last_error') ?: null;
    }
